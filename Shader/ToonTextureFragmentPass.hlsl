#ifndef TOON_TEXTURE_FRAG_PASS_INCLUDED
#define TOON_TEXTURE_FRAG_PASS_INCLUDED

#include "Standard/ToonStandardSurface.hlsl" 

half4 ToonTexturePassFragment(Varyings input) : SV_Target
{
    ...
    half3 rampedMainLight = SAMPLE_TEXTURE2D(_MainRamp, sampler_MainRamp, clamp(float2(mainIllumination * surfaceData.occlusion, 0.0f), 0.01f, 0.99f)).xyz;
    ...
    for (int i = 0; i < pixelLightCount; ++i)
    {
        ...
        half3 alightColor = additionalLight.color * additionalLight.distanceAttenuation * additionalLight.shadowAttenuation;
        ...
        half3 rampedAdditionalLight = SAMPLE_TEXTURE2D(_AddRamp, sampler_AddRamp, clamp(float2(additionalIllumination * surfaceData.occlusion, 0.0f), 0.05f, 0.95f)).xyz;
#ifndef _BACKLIGHT_OFF
    ...
    light += ToonAdditionalVertexLightDiffuse(inputData.vertexLighting, surfaceData.occlusion, _AddRamp, sampler_AddRamp);
#endif
