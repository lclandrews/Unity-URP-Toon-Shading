...
namespace UnityEditor.Rendering.Universal.ShaderGUI
{
    internal class ToonMasterShader : BaseShaderGUI
    {
        ...
        public override void DrawSurfaceInputs(Material material)
        {
            ...
            StandardToonGUI.DrawSpecularArea(toonStandardProperties, materialEditor, material, ValidateMaterial);
            DrawNormalArea(materialEditor, toonStandardProperties.bumpMapProp, toonStandardProperties.bumpScaleProp);
            StandardToonGUI.DrawBacklightArea(toonStandardProperties, alphaClipProp, materialEditor, material, ValidateMaterial);
            StandardToonGUI.DrawEdgeShineArea(toonStandardProperties, alphaClipProp, materialEditor, material, ValidateMaterial);
        ...
        {
            EditorGUI.BeginChangeCheck();
            Color ambient = ToonMainGUI.DrawColorField(toonStandardProperties.ambientColorProp.colorValue, ToonMainGUI.Styles.ambientColorText, true, false, false);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            Gradient mainGrad = ToonMainGUI.ShaderValuesToGradient(mainGradType, mainGradColors, mainGradArray);
            EditorGUI.BeginChangeCheck();
            mainGrad = ToonMainGUI.DrawMiniGradientField(mainGrad, ToonMainGUI.Styles.mainGradText, false);

            if (EditorGUI.EndChangeCheck())
            {
                ToonMainGUI.GradientToShaderValues(mainGrad, out mainGradType, out mainGradColors, out mainGradArray);
            ...
            Gradient addGrad = ToonMainGUI.ShaderValuesToGradient(addGradType, addGradColors, addGradArray);
            EditorGUI.BeginChangeCheck();
            addGrad = ToonMainGUI.DrawMiniGradientField(addGrad, ToonMainGUI.Styles.addGradText, false);

            if (EditorGUI.EndChangeCheck())
            {
                ToonMainGUI.GradientToShaderValues(addGrad, out addGradType, out addGradColors, out addGradArray);
